...
static ENetProtocolCommand enet_protocol_remove_sent_reliable_command(
  ...
  commandNumber = (ENetProtocolCommand)(
      outgoingCommand->command.header.command & ENET_PROTOCOL_COMMAND_MASK);
  ...
  for (currentPeer = host->peers; currentPeer < &host->peers[host->peerCount];
    ...
    if (currentPeer->state == ENET_PEER_STATE_DISCONNECTED) {
      if (peer == NULL) peer = currentPeer;
               ...
               enet_host_equal(currentPeer->address.host,
...
/* フラグメンテーションを起こしたコマンドを１つずつ処理する（断片ごとに
 * enet_protocol_handle_send_fragment() を呼ぶ）*/
...
static int enet_protocol_handle_send_fragment(ENetHost *host, ENetPeer *peer,
  ...
  *currentData += fragmentLength;
  ...
  // ▶ フラグメントの所属ウィンドウ（startWindow）が currentWindow
  // 未満の場合は既に受信したフラグメントなので破棄する ▶
  ...
  for (currentCommand = enet_list_previous(
    ...
    // startSequenceNumber が incomingReliableSequenceNumber
    // に等しい：先頭セグメントは受信済み startSequenceNumber が
    // incomingReliableSequenceNumber より大きい：先頭セグメント未受信
    if (startSequenceNumber >= channel->incomingReliableSequenceNumber) {
      //
    ...
    // reliableSequenceNumber が startSequenceNumber
    // に等しければ、先頭セグメントとして扱う
...
static int enet_protocol_handle_send_unreliable_fragment(
    ENetHost *host, ENetPeer *peer, const ENetProtocol *command,
    enet_uint8 **currentData) {
  enet_uint32 fragmentNumber, fragmentCount, fragmentOffset, fragmentLength,
  ...
  *currentData += fragmentLength;
  ...
  // fragmentCount = 10 なら fragments のサイズは 0 となる（fragments は
  // enet_uint32 の配列なので、１要素で 32個まで断片を管理できる）
  // fragmentNumber = 0 のとき、fragments のインデックスは 0 で、シフトは 0
  // ビットの左シフト なので、fragments[0] の 00000000 00000000 00000000
  // 00000000 と 00000000 00000000 00000000 00000001 の論理演算となる 結果が 0
  // なら、その断片は初めて処理するものなので、データ領域にコピーする
  if ((startCommand->fragments[fragmentNumber / 32] &
       (1 << (fragmentNumber % 32))) == 0) {
    --startCommand->fragmentsRemaining;
...
static int enet_protocol_handle_throttle_configure(
    ENetHost *host, ENetPeer *peer, const ENetProtocol *command) {
  if (peer->state != ENET_PEER_STATE_CONNECTED &&
      peer->state != ENET_PEER_STATE_DISCONNECT_LATER)
    return -1;
...
static int enet_protocol_handle_incoming_commands(ENetHost *host,
  ...
  if (host->checksum != NULL) headerSize += sizeof(enet_uint32);
  ...
  if (flags & ENET_PROTOCOL_HEADER_FLAG_COMPRESSED) {
    ...
    originalSize =
        host->compressor.decompress(host->compressor.context,
  ...
  while (currentData < &host->receivedData[host->receivedDataLength]) {
    ...
    if (peer == NULL && commandNumber != ENET_PROTOCOL_COMMAND_CONNECT) break;
    ...
    switch (commandNumber) {
      ...
      case ENET_PROTOCOL_COMMAND_CONNECT:
        if (peer != NULL) goto commandError;
...
commandError:
  if (event != NULL && event->type != ENET_EVENT_TYPE_NONE) return 1;
...
static int enet_protocol_receive_incoming_commands(ENetHost *host,
  ...
  for (packets = 0; packets < 256; ++packets) {
    ...
    // - packetDataの先頭ポインタを渡しているので、受信したデータは host ->
    // packetData で参照できるようになる
    // - buffer.data と host -> receivedData は、ともに host -> packetData [0]
    // を参照する
...
#ifdef ENET_DEBUG
        printf(
            "peer %u: %f%%+-%f%% packet loss, %u+-%u ms round trip time, %f%% "
      ...
      if (host->compressor.context != NULL &&
        ...
        size_t originalSize = host->packetSize - sizeof(ENetProtocolHeader),
               compressedSize = host->compressor.compress(
                   ...
                   &host->buffers[1],  // in
                   host->bufferCount - 1, originalSize,
