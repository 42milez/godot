...
void *enet_range_coder_create(void) {
  ...
  if (rangeCoder == NULL) return NULL;
...
#define ENET_CONTEXT_ENCODE_EXCLUDE(context, value_, under, total, minimum) \
  ENET_CONTEXT_WALK(context, {                                              \
    if (node->value != value_) {                                            \
      ...
      if (node->value < value_) under -= parentCount;                       \
      total -= parentCount;                                                 \
    }                                                                       \
  })
#endif
  ...
  {                                                         \
    ...
    for (;;) {                                              \
      ...
      if (value_ < exclude->value) {                        \
        ...
        if (exclude > excludes) after -= exclude[-1].under; \
      } else {                                              \
        if (value_ > exclude->value) {                      \
          if (low + 1 < high) {                             \
            low = mid;                                      \
...
size_t enet_range_coder_decompress(void *context, const enet_uint8 *inData,
  ...
  if (rangeCoder == NULL || inLimit <= 0) return 0;
