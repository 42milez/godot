...
int enet_peer_send(ENetPeer *peer, enet_uint8 channelID, ENetPacket *packet) {
  ...
  if (peer->host->checksum != NULL) fragmentLength -= sizeof(enet_uint32);
  ...
  if (packet->dataLength > fragmentLength) {
    ...
    for (fragmentNumber = 0, fragmentOffset = 0;
      ...
      // 後で packet の referenceCount を増やすために、一旦 fragments
      // にまとめている
    ...
    // C++
    // ならスマートポインタで参照カウントを増やせるので、ループ処理の中で直接
    // enet_peer_setup_outgoing_command() を呼べる
    while (!enet_list_empty(&fragments)) {
      fragment =
...
ENetPacket *enet_peer_receive(ENetPeer *peer, enet_uint8 *channelID) {
  ...
  if (channelID != NULL) *channelID = incomingCommand->command.header.channelID;

  packet = incomingCommand->packet;

  --packet->referenceCount;

  if (incomingCommand->fragments != NULL) enet_free(incomingCommand->fragments);
...
void enet_peer_setup_outgoing_command(ENetPeer *peer,
  ...
  // チャネル数が最大の場合
  // enet_host_connectのタイミングではchannelIDが0xFFだが、sizeof(channels) <
  // 0xFFでも、 この条件があるので領域外アクセスは起こらない
...
ENetOutgoingCommand *enet_peer_queue_outgoing_command(
    ENetPeer *peer, const ENetProtocol *command, ENetPacket *packet,
    enet_uint32 offset, enet_uint16 length) {
  ENetOutgoingCommand *outgoingCommand =
      (ENetOutgoingCommand *)enet_malloc(sizeof(ENetOutgoingCommand));
  if (outgoingCommand == NULL) return NULL;
...
void enet_peer_dispatch_incoming_unreliable_commands(ENetPeer *peer,
  ...
  for (droppedCommand = startCommand = currentCommand =
    ...
    if ((incomingCommand->command.header.command &
         ENET_PROTOCOL_COMMAND_MASK) == ENET_PROTOCOL_COMMAND_SEND_UNSEQUENCED)
      continue;
...
void enet_peer_dispatch_incoming_reliable_commands(ENetPeer *peer,
  ...
  // 新しいコマンドのポインタをコピーする（関数名には move
  // とあるが、実体の移動ではなく、あくまでもポインタのコピー）
...
ENetIncomingCommand *enet_peer_queue_incoming_command(
    ENetPeer *peer, const ENetProtocol *command, const void *data,
  ...
  if (packet == NULL) goto notifyError;

  incomingCommand =
      (ENetIncomingCommand *)enet_malloc(sizeof(ENetIncomingCommand));
  if (incomingCommand == NULL) goto notifyError;
